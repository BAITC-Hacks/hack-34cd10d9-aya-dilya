      ...
      <div className="workspace">
        ...
        <section className="results-panel" id="results" aria-labelledby="results-title"><div className="section-heading"><span className="step">02</span><h2 id="results-title">Ваши варианты</h2><span className="result-count">{outcome && !loading && !error ? `${outcome.response.cards.length} / 3` : 'ДО 3 ВАРИАНТОВ'}</span></div>
          ...
          {loading ? <div className="empty-state" aria-busy="true"><span className="loading-symbol" aria-hidden="true">✳</span><h3>Ищем подходящих людей</h3><p>Сверяем условия с каталогом и календарём.</p></div> : error ? <div className="error-box" role="alert"><h3>Не удалось выполнить подбор</h3><p>Ваши условия сохранены. Попробуйте ещё раз.</p><button onClick={() => formRef.current?.requestSubmit()}>Повторить подбор</button></div> : outcome ? <>
            ...
            {outcome.response.status === "matched" ? <><ContractorComparison cards={outcome.response.cards} ai={outcome.response.ai?.mode === "openai"} /><div className="cards">{outcome.response.cards.map((card, index) => <ContractorCard key={card.id} card={card} index={index} ai={outcome.response.ai?.mode === "openai"} />)}</div></> : <div className="empty-state no-match"><span aria-hidden="true">↗</span><h3>{outcome.response.status === "category_absent" ? "В этом городе пока нет такой категории" : "По этим условиям никто не подходит"}</h3><p>{outcome.response.status === "category_absent" ? "Попробуйте выбрать другой город или категорию." : "Попробуйте другую дату, пересмотрите бюджет или необязательные условия: язык и длительность."}</p><p>Мы не меняли ваши параметры.</p>{outcome.response.status === "no_match" && !outcome.response.alternatives?.length && <p>Проверили другие даты во всём доступном календаре и изменение бюджета на выбранную дату. Одного изменения недостаточно. Можно пересмотреть несколько условий: дату вместе с бюджетом, формат, язык или длительность.</p>}<button className="text-button" onClick={() => document.getElementById(outcome.response.status === "category_absent" ? "category" : "budgetKzt")?.focus()}>Изменить условия <span aria-hidden="true">↗</span></button></div>}
            {outcome.response.status === "no_match" && !!outcome.response.alternatives?.length && <section className="plan-b" aria-label="План Б">
              ...
              {outcome.response.alternatives.map(option => <article className="alternative" key={option.id}>
                <span className="alternative-label">{option.kind === "budget" ? (outcome.request.budgetKzt > 0 && option.request.budgetKzt - outcome.request.budgetKzt <= outcome.request.budgetKzt * 0.2 ? "Немного больше бюджет" : "Другой бюджет") : "Ближайшая подходящая дата"}</span><h4>{option.title}</h4><p>{option.description}</p>
