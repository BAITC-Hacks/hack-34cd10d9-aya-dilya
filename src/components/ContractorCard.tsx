...
export function ContractorCard({ card, index, ai = false }: { card: RecommendationCard; index: number; ai?: boolean }) {
  return <article className="contractor-card" data-card-id={card.id}>
    ...
    {!!card.evidence?.length && <div className="evidence"><h4>Что подтверждает пожелания</h4>{card.evidence.map(item => <figure key={item.id}><blockquote>{item.quote}</blockquote><figcaption>{item.label} · Цитата из описания профиля</figcaption></figure>)}</div>}
