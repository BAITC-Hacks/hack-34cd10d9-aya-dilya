...
function Harness() {
  ...
  return <><div style={{ padding: 16, background: "#ffe4a0" }}><strong>Тестовый стенд UI: искусственные задержки и ошибки, не продуктовый режим.</strong><label htmlFor="test-mode"> Режим проверки </label><select id="test-mode" style={{ maxWidth: 300 }} value={mode} onChange={e => setMode(e.target.value)}><option value="delay">Задержка первого ответа</option><option value="error">Техническая ошибка</option><option value="stale-error">Устаревшая ошибка</option><option value="catalog-error">Ошибка справочников</option></select></div><RecommendationPage key={mode} service={service} /></>;
}
